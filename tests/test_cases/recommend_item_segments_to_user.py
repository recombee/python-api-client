...
class RecommendItemSegmentsToUserTest(RecombeeTest, ABC):
    ...
    def test_recommend_item_segments_to_user(self):

        # it 'rejects request to scenario which is not set up'
        req = self.create_request('entity_id', 5, scenario='scenario1', cascade_create=True)
        try:
